...
import { getTerritory, isPointInTerritory } from "./territories.js";
...
export function normalizeFireRow(
  row,
  source,
  now = new Date(),
...
export function annotateEstimatedStarts(fires, clusterRadiusKm = 5) {
  return fires.map((fire) => {
    const nearbyTimes = fires
      .filter((candidate) => haversineKm(fire, candidate) <= clusterRadiusKm)
      .map((candidate) => new Date(candidate.observedAt).getTime())
      .filter(Number.isFinite);
    const earliest = nearbyTimes.length ? Math.min(...nearbyTimes) : new Date(fire.observedAt).getTime();
    return {
      ...fire,
      estimatedStartAt: new Date(earliest).toISOString(),
      startEstimateRadiusKm: clusterRadiusKm,
    };
  });
}
...
async function fetchSource({ mapKey, source, days, territory, fetchImpl }) {
  const bbox = [
    territory.queryBounds.west,
    territory.queryBounds.south,
    territory.queryBounds.east,
    territory.queryBounds.north,
  ].join(",");
  ...
  const now = new Date();
  const fires = rows
    .map((row) => normalizeFireRow(row, source, now, territory))
    .filter(Boolean);
  return { source, fires };
}
...
export async function fetchFirmsFires({
  ...
  if (!territory) throw new Error("Territorio non supportato.");
  const results = await Promise.allSettled(
    sources.map((source) =>
      fetchSource({ mapKey, source, days, territory, fetchImpl }),
    ),
  );
  ...
  const normalized = annotateEstimatedStarts(deduplicateFires(fires)).sort(

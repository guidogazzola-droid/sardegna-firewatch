...
function notificationMessage(subscription, fires) {
  ...
  const countText = fires.length > 1 ? `${fires.length} nuove rilevazioni` : "Una nuova rilevazione";
  return {
    to: subscription.expoPushToken,
    sound: "default",
    title: "Sabetta Piro — rilevazione satellitare",
    body: `${countText} entro la zona monitorata; la più vicina è a circa ${roundedDistance(
      subscription,
      primary,
    )} km. Verifica la mappa e le comunicazioni delle autorità.`,
    data: {
      type: "fire-detection",
      fireId: primary.id,
...
export class AlertMonitor {
  ...
  async runOnce(now = new Date()) {
    ...
    try {
      ...
      const fires = await this.fetchFires();
      const outgoing = [];
